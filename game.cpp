...
void set_player_preferences(int& dim_x, int& dim_y, char& team) {
	...
	while (repeat) {
		try {
			...
			cout << "Please enter the dimensions of the map: " << endl;
			cin >> x;
			cin >> y;
	
			if (x.length() >= 3 || y.length() >= 3) {		// If the input has more than 3 digits/characters is surely wrong
				throw(ERROR_CHAR_INPUT);					// in that way an overflow is avoided
			}
